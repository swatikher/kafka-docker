...
//Setup and configure StreamExecution Environment
final StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();

//Create a DataStream that reads InputFile with FUTURE_TRADES data
DataStream<Tuple5<String, String, String, Double, Integer>> data = env.readTextFile("/tmp/FUTURES_TRADES.txt")
...
// Create a tuple and parse = date,    time,     Name,       trade,    volume
...
// Compute per window statistics -- call method TrackChange -- and track if any change in price occurred based on Thresholds
// store the output results in file FirstReport.txt
  ...
  change.writeAsText("/tmp/Firstreport.txt");
  change.print();


// Check for LargeDelta - create an Alert when price change from one window to another is more than threshold
...
largeDelta.writeAsText("/tmp/Alert.txt");
largeDelta.print();
env.execute("Stock Analysis");
  }


//TrackChange will take into account values for MaxTrade, MinTrade, MaxVolume, and MinVolume
